...
namespace UnityEngine.InventorySystem
{
    ...
    public class ItemEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            researchField?.SetValue(item, (int)EditorGUILayout.Slider("Coins:", (int)researchField?.GetValue(item), 0, 24));

            FieldInfo resourceField = item.GetType().GetField("_hearts", _flags);
            resourceField?.SetValue(item, (int)EditorGUILayout.Slider("Hearts:", (int)resourceField?.GetValue(item), 0, 24));
